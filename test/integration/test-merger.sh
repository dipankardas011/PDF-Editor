...
# create container merger
cd ../../src/backend/merger
docker build --target prod -t backend .

# create container rotator
cd ../rotator
docker build --target prod -t rotate .
...
Call_Cleanup() {
  ...
  docker rm -f frontend backend-merge backend-rotate
  docker rmi -f rotate frontend backend
  cd ../../src/frontend/
...
echo -e "\n$(tput setaf 3)$(tput bold)- /merge/upload$(tput init)"
echo -e "$(tput setaf 3)$(tput bold)- /merge/download$(tput init)"
echo -e "\n$(tput setaf 3)$(tput bold)- /rotate/upload$(tput init)"
echo -e "$(tput setaf 3)$(tput bold)- /rotate/download$(tput init)"
...
RotatorTestCases() {
  # @@ TESTING ALL Rotator ENDPOINTS

  echo "----------------------------------------------------------------"

  echo -e "\n$(tput setaf 5)$(tput bold)Testing Endpoint '/rotate/upload'$(tput init)"
  flag=0
  echo -e "\n$(tput setaf 6)$(tput bold)  ↕ Upload file 01.pdf$(tput init)"
  # sleep 100
  hello=$(curl --raw --form "myFile=@${PWD}/resources/01.pdf" --form "pages=," http://localhost:${PORT}/rotate/upload | grep -e '<div.*;alert-success role="alert">Uploaded.*<\/div>' | wc -l)
  if [[ $hello -eq 0 ]]; then
  ...
  if [[ $flag -eq 1 ]]; then
    echo -e "\n$(tput setaf 1)$(tput bold)✗ [Failed] the test of Endpoint /rotate/upload$(tput init)"
  else
    counterEndpoints=$((counterEndpoints+1))
    isUploadSuccess=0
    echo -e "\n$(tput setaf 2)$(tput bold)✓ [Passed] the test of Endpoint /rotate/upload$(tput init)"
  fi

  echo "----------------------------------------------------------------"

  echo -e "\n$(tput setaf 5)$(tput bold)Testing Endpoint '/rotate/upload & /rotate/download'$(tput init)"

  if [[ $isUploadSuccess -eq 1 ]]; then
    echo -e "\n$(tput setaf 1)$(tput bold)✗ [Failed] the test of Endpoint /rotate/upload so [FAILED]$(tput init)"
  else
    hello=$(curl --raw --output rotated.pdf -X GET http://localhost:$PORT/rotate/download && cat rotated.pdf | head -n 5 | grep -be "%PDF.*" | wc -l) # return value is stored in hello

    if [[ $hello -gt 0 ]]; then
      counterEndpoints=$((counterEndpoints+1))
      echo -e "\n$(tput setaf 2)$(tput bold)✓ [Passed] the test of Endpoint /rotate/download$(tput init)"
    else
      echo -e "\n$(tput setaf 1)$(tput bold)✗ [Failed] the test of Endpoint /rotate/download$(tput init)"
    fi
  fi
}
